import { Box, Button, ButtonGroup, Card, CardContent, Checkbox, Dialog, DialogContent, IconButton, TableBody, TableCell, TableHead, TablePagination, TableRow, Tooltip, Typography } from "@mui/material"
...
import { ArrowLeft, Delete, Edit, Group } from "@mui/icons-material"
import { AdicionarContatosCampanha, DeletarContatoCampanha, DeletarTodosContatosCampanha, ListarContatosCampanha } from "../../services/campanhas"
...
export const ContatosCampanha = () => {
    ...
    const handleRowsPerPageChangeModal = (
        ...
        setPaginationModal({ page: 0, rowsPerPage: Number.parseInt(event.target.value, 10) });
    };
    ...
    const handleSelectContact = (contato) => {
        ...
        if (contatosSelected.some(selectedContato => selectedContato.id === contato.id)) {
            // Remove o contato se já estiver selecionado
            setContatosSelected(contatosSelected.filter(selectedContato => selectedContato.id !== contato.id));
        } else {
    ...
    const handleRemoverContatoCampanha = async (contato) => {
        DeletarContatoCampanha(campanhaId, contato.id)
            .then(() => {
    ...
    const columns = [
        {
            name: 'profilePicUrl', label: '', field: 'profilePicUrl', renderCell: ({ value }) => (
                <img
                    src={value}
                    alt="Profile"
        ...
        { name: 'number', label: 'WhatsApp', field: 'number' },
        {
            ...
            renderCell: ({ value }) => value.length > 0 ? ` ${ack[value[0].ack] || 'N/A'}` : 'N/A'
        ...
        },
        {
            ...
            renderCell: ({ value }) => {
                if (value && value.length > 0) {
                    const tagsStr = value.map(tagObj => tagObj.tag).join(', ');
        ...
        {
            name: 'estado', label: 'Estado', field: 'number',
            renderCell: ({ value }) => definirEstadoNumero(value)
        },
        {
    ...
    const columnsAdd = [
        { name: 'checkbox', label: <Checkbox checked={selectAll} onChange={handleSelectAll} />, field: 'checkbox' },
        ...
        toast.info(
            'Atenção!! Deseja realmente retirar todos os contatos desta campanha? ',
            {
                ...
                action: {
                    label: 'Confirma',
                    onClick: () => {
                        DeletarTodosContatosCampanha(campanhaId)
                            .then(() => {
                                listarContatosCampanha()
                                toast.info('Contato excluído desta campanha')
                            })
                                ...
                                <TableRow key={contato.id}>
                                    {columns.map(col => (
                                        ...
                                        >
                                            ...
                                            {col.renderCell
                                                ? col.renderCell({ value: contato[col.field], contato }) // Usa o renderCell se estiver definido
                                                : (typeof contato[col.field] === 'object' && contato[col.field] !== null
                    ...
                    <DialogContent>
                                ...
                                {contatos.map(contato => (
                                    <TableRow key={contato.id}>
                                        {columnsAdd.map(col => (
                                            <TableCell
                                                key={col.name}
                                                sx={{ width: `${100 / columnsAdd.length}%`, flexGrow: 1, textAlign: 'center', }}
                                            >
                                                {col.name === 'checkbox' ? (
                                                    <Checkbox
                                                        checked={contatosSelected.some(selectedContato => selectedContato.id === contato.id)} // Verifica se o contato já está selecionado
                        ...
                        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                            {contatosSelected.length > 0 ?
                                <Typography>{contatosSelected.length} registro selecionado.</Typography> :
                                <Box />
                            }
                            <TablePagination
