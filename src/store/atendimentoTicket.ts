...
const checkTicketFilter = (ticket: Ticket) => {
  ...
  const filtros =
    JSON.parse(localStorage.getItem('filtrosAtendimento') || '{}') ||
    filtroPadrao
...
>((set, get) => ({
  ...
  updateTicket: payload =>
    set(state => {
      ...
      if (ticketIndex !== -1) {
        ...
        if (state.ticketFocado && state.ticketFocado.id === payload.id) {
          return {
  ...
  updateMessages: payload =>
    set(state => {
      if (state.ticketFocado.id === payload.ticket.id) {
        ...
        const messageIndex = updatedMessages.findIndex(m => m.id === payload.id)
        if (messageIndex !== -1) {
  ...
  updateMessageStatus: payload => {
    ...
    // Se o ticket focado não for o mesmo que o payload, não atualiza.
    if (ticketFocado?.id !== payload.ticket.id) return

    // Encontra o índice da mensagem que precisa ser atualizada.
    const messageIndex = mensagens.findIndex(m => m.id === payload.id)

    // Cria uma cópia das mensagens para evitar mutações diretas.
    const updatedMensagens = [...mensagens]

    // Se a mensagem for encontrada, atualiza-a.
    if (messageIndex !== -1) {
    ...
    // Atualiza as mensagens agendadas, se existirem
  ...
  AbrirChatMensagens: async data => {
    try {
      ...
      toast.error(
        `Ops... Ocorreu um problema não identificado. ${JSON.stringify(error)}`
      )
    }
  },
}))
